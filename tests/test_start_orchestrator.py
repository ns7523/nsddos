...
def _healthy_services() -> tuple[StartupServiceStatus, ...]:
    return (
        StartupServiceStatus(
            "labhost", "nsddos-labhost", "running", "healthy", True, "1"
        ),
        StartupServiceStatus(
            "floodlight", "nsddos-floodlight", "running", "healthy", True, "2"
        ),
        StartupServiceStatus(
            "sflowrt", "nsddos-sflowrt", "running", "healthy", True, "3"
        ),
        StartupServiceStatus(
...
def test_detect_compose_backend_prefers_docker_compose(monkeypatch) -> None:
    monkeypatch.setattr(
        "nsddos.bootstrap.stack.resolve_compose_command", lambda: ("docker", "compose")
    )

    backend = detect_compose_backend()

    assert backend == ComposeBackend(
        name="docker-compose-v2", command=("docker", "compose")
    )


def test_detect_compose_backend_falls_back_to_docker_compose_binary(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        "nsddos.bootstrap.stack.resolve_compose_command", lambda: ("docker-compose",)
    ...
    assert backend == ComposeBackend(
        name="docker-compose-v1", command=("docker-compose",)
    )
...
def test_ensure_startup_prerequisites_uses_installer_subset(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "nsddos.bootstrap.runtime_boot.execute_install_plan", fake_execute
    )
...
def test_wait_for_stack_health_times_out_with_pending_services(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "nsddos.bootstrap.healthwait.list_stack_services", lambda backend: services
    )
...
def test_orchestrator_detects_already_running_and_persists_session(
    ...
    monkeypatch.setattr(
        "nsddos.bootstrap.orchestrator.DEFAULT_STARTUP_PROFILE", profile
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "nsddos.bootstrap.orchestrator.detect_compose_backend",
        lambda: ComposeBackend("docker-compose-v2", ("docker", "compose")),
    )
    monkeypatch.setattr(
        "nsddos.bootstrap.orchestrator.list_stack_services",
        lambda backend: _healthy_services(),
    )
    monkeypatch.setattr("nsddos.bootstrap.orchestrator.ui_reachable", lambda url: True)
    monkeypatch.setattr(
        "nsddos.bootstrap.orchestrator.validate_runtime_health", lambda: ((), ())
    )
    monkeypatch.setattr(
        "nsddos.bootstrap.orchestrator.launch_ui_background",
        lambda: (_ for _ in ()).throw(AssertionError("should not launch ui")),
    )
...
def test_orchestrator_persists_successful_startup_session(
    monkeypatch, tmp_path
) -> None:
    profile = StartupProfile(
        container_names=(
    ...
    monkeypatch.setattr(
        "nsddos.bootstrap.orchestrator.list_stack_services", lambda backend: ()
    )
    monkeypatch.setattr(
        "nsddos.bootstrap.orchestrator.start_stack",
        lambda backend, rebuild: subprocess.CompletedProcess(
            ["docker"], 0, stdout="started", stderr=""
        ),
    )
    monkeypatch.setattr(
...
def test_run_startup_command_renders_boot_monitor_without_panel_matrix(
    monkeypatch,
) -> None:
    ...
    def fake_orchestrate(_console, status_callback=None):
        ...
        return type(
            "Result", (), {"failed_checks": (), "ui_url": "http://127.0.0.1:8010"}
        )()
