...
def test_runtime_executor_executes_lab_via_docker_exec(monkeypatch) -> None:
    ...
    result = RuntimeExecutor(lab_container="nsddos-labhost").execute_lab(
        ["mn", "--version"], timeout=10
    )
...
def test_runtime_executor_parses_lab_link_index_map(monkeypatch) -> None:
    monkeypatch.setattr(
        "nsddos.runtime.executor.RuntimeExecutor.lab_container_running",
        lambda self: True,
    )
    monkeypatch.setattr(
...
def test_detect_runtime_capabilities_prefers_container_runtime(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "nsddos.runtime.capabilities.DockerManager.is_docker_installed",
        staticmethod(lambda: True),
    )
    monkeypatch.setattr(
        "nsddos.runtime.capabilities.DockerManager.is_daemon_running", lambda self: True
    )
    monkeypatch.setattr(
        "nsddos.runtime.capabilities.RuntimeExecutor.lab_container_running",
        lambda self: True,
    )
