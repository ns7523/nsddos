...
def test_docker_manager_uses_shared_resolver(monkeypatch) -> None:
    monkeypatch.setattr(
        "nsddos.docker_manager.resolve_compose_command", lambda: ("docker-compose",)
    )

    assert DockerManager._compose_backend() == ["docker-compose"]


def test_docker_manager_run_uses_resolved_compose_backend(
...
def test_docker_manager_get_service_states_falls_back_to_docker_ps(
    monkeypatch, tmp_path
) -> None:
    compose_file = tmp_path / "docker-compose.yml"
    compose_file.write_text("services: {}\n", encoding="utf-8")
    ...
    monkeypatch.setattr(
        "nsddos.docker_manager.DockerManager.is_docker_installed",
        staticmethod(lambda: True),
    )
    monkeypatch.setattr(
        "nsddos.docker_manager.DockerManager.is_daemon_running", lambda self: True
    )
    ...
    def fake_run(argv, **kwargs):
        if tuple(argv) == (
            ...
            return subprocess.CompletedProcess(
                argv, 1, stdout="", stderr="unknown flag: --format"
            )
...
def test_docker_manager_get_service_states_uses_compose_json_when_available(
    monkeypatch, tmp_path
) -> None:
    compose_file = tmp_path / "docker-compose.yml"
    compose_file.write_text("services: {}\n", encoding="utf-8")
...
def test_list_stack_services_uses_docker_manager_fallback(
    monkeypatch, tmp_path
) -> None:
    compose_file = tmp_path / "docker-compose.yml"
    compose_file.write_text("services: {}\n", encoding="utf-8")
...
def test_list_stack_services_prefixes_compose_service_name(
    monkeypatch, tmp_path
) -> None:
    compose_file = tmp_path / "docker-compose.yml"
    compose_file.write_text("services: {}\n", encoding="utf-8")
...
def test_tracked_runtime_code_has_single_compose_probe_site() -> None:
    ...
    for path in checked:
        ...
        if (
            path.name != "compose.py"
            and '("docker", "compose")' in text
            and "ComposeBackend" not in text
        ):
            offenders.append(str(path))
    assert offenders == []
