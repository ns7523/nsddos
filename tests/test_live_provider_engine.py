...
def test_sflow_telemetry_ingestion():
    provider = type(
        "Provider", (), {"api_url": "http://sflow", "is_reachable": lambda self: True}
    )()
    pool = _Pool(
        {
            ...
            "http://sflow/metric/ALL/json": {
                "ok": True,
                "payload": [{"drops": 2}],
                "latency_ms": 10.0,
            },
            "http://sflow/topology/json": {
                "ok": True,
                "payload": {},
                "latency_ms": 9.0,
            },
...
def test_ovs_telemetry_ingestion():
    bridge = type(
        "Bridge",
        (),
        {"name": "s1", "interfaces": ["s1-eth1"], "controller_connected": True},
    )()
    state = type(
        "State", (), {"installed": True, "service_running": True, "bridges": [bridge]}
    )()
...
def test_floodlight_controller_collection():
    ...
    pool = _Pool(
        {
            "http://flood/wm/core/controller/switches/json": {
                "ok": True,
                "payload": [{"switchDPID": "00:01"}],
                "latency_ms": 8.0,
            },
            "http://flood/wm/device/": {
                "ok": True,
                "payload": [{"ipv4": ["10.0.0.1"]}],
                "latency_ms": 7.0,
            },
            "http://flood/wm/topology/links/json": {
                "ok": True,
                "payload": [{"src-switch": "00:01", "dst-switch": "00:02"}],
                "latency_ms": 6.0,
            },
        }
    )
...
def test_ovs_provider_installs_and_matches_drop_flow(monkeypatch):
    ...
    def _run(args, require_root=False, timeout=5):
        ...
        if args[:3] == ["-O", "OpenFlow13", "dump-flows"] and args[3] == "s1":
            stdout = (
                " cookie=0x0, duration=1.0s, table=0, n_packets=0, n_bytes=0, priority=50000,ip,nw_src=10.0.0.8 actions=drop\n"
                if state["installed"]
                else ""
            )
    ...
    assert (
        provider.install_drop_flow(
            "s1", "priority=50000,ip,nw_src=10.0.0.8,actions=drop"
        )
        is True
    )
...
def test_timeout_handling():
    pool = DeterministicConnectionPool(
        ConnectionPolicy(timeout_seconds=0.01, retry_count=0)
    )
...
def test_malformed_telemetry_rejection():
    snapshot = LiveTelemetrySnapshot(
        ...
        active_flows=1,
        dropped_packets=0,
        topology_state=TopologySnapshot(
...
def test_provider_reconnection_logic():
    health = evaluate_provider_health(
        "sflowrt", reachable=False, latency_ms=0.0, malformed=False, error_count=2
    )
...
def test_deterministic_telemetry_normalization(monkeypatch):
    ...
    monkeypatch.setattr(
        telemetry_module,
        "collect_ovs_telemetry",
        lambda provider: {
            "reachable": True,
            "latency_ms": 0.0,
            "bridges": ("s1",),
            "interfaces": ("s1-eth1",),
            "active_connection_state": True,
            "dropped_packets": 0,
            "malformed": False,
        },
    )
    ...
    monkeypatch.setattr(
        telemetry_module,
        "collect_floodlight_telemetry",
        lambda provider, pool: {
            "reachable": True,
            "latency_ms": 0.0,
            "controller_port_open": True,
            "switches": ("s1",),
            "hosts": ("10.0.0.1",),
            "links": (),
            "malformed": False,
    ...
    monkeypatch.setattr(
        ...
        type(
            "FakeDateTime",
            (),
            {"now": staticmethod(lambda tz=None: snapshot_time), "timezone": timezone},
        ),
    )
...
def test_collection_path_unchanged_when_live_disabled(monkeypatch):
    ...
    monkeypatch.setattr(
        collectors_module, "runtime_registry", lambda config: {"x": object()}
    )
    monkeypatch.setattr(
        collectors_module,
        ...
        lambda registry: {
            "sflowrt": {
                ...
                "topology_accessible": True,
            }
        },
    )
    monkeypatch.setattr(
        collectors_module,
        "sample_flow_visibility",
        lambda config, interval=1.0: collectors_module.FlowState(
            flow_count=1, telemetry_present=True
        ),
    )
    monkeypatch.setattr(
        collectors_module,
        "telemetry_freshness",
        lambda config, interval=1.0: collectors_module.TelemetryFreshness(
            last_flow_timestamp="2100-01-01T00:00:00+00:00", sample_interval_seconds=1.0
        ),
    )
    monkeypatch.setattr(
        collectors_module,
        "normalize_controller_topology",
        lambda config: type("C", (), {"to_dict": lambda self: {}})(),
    )
    monkeypatch.setattr(
        collectors_module,
        "detect_runtime_profile",
        lambda: type("P", (), {"to_dict": lambda self: {"name": "linux-native"}})(),
    )
    monkeypatch.setattr(
        collectors_module,
        "detect_runtime_capabilities",
        lambda: type("C", (), {"to_dict": lambda self: {"docker_daemon": True}})(),
    )
    monkeypatch.setattr(
        collectors_module,
        "validate_runtime_environment",
        lambda config: type(
            "E", (), {"to_dict": lambda self: {"status": "compatible"}}
        )(),
    )
    monkeypatch.setattr(
        collectors_module,
        ...
        lambda config: type(
            "R", (), {"to_dict": lambda self: {"status": "reproducible"}}
        )(),
    )

    bundle = collectors_module.collect_runtime_state(
        {"runtime": {"live": {"enabled": False}}}
    )
...
def test_live_snapshot_feeds_collection_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr(
        collectors_module, "collect_live_telemetry", lambda config: snapshot
    )
    monkeypatch.setattr(
        collectors_module,
        "live_snapshot_to_collection_state",
        lambda item: live_snapshot_to_collection_state(snapshot),
    )
    monkeypatch.setattr(
        collectors_module,
        "normalize_controller_topology",
