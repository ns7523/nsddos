...
def test_demo_runs_live_flow_with_summary(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _console: SimpleNamespace(
            failed_checks=(), ui_url="http://127.0.0.1:8010"
        ),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: {
            ...
            "scenarios": [{"attack_type": "udp_flood"}],
        },
    )
    ...
    mitigation_plan = SimpleNamespace(
        mitigation_required=True, mitigation_action="rate_limit"
    )
    ...
    monkeypatch.setattr(
        "nsddos.cli.evaluate_mitigation",
        lambda _config, detection=None: mitigation_plan,
    )
    monkeypatch.setattr(
        "nsddos.cli.enforce_mitigation", lambda _config, _evaluation: mitigation_result
    )
