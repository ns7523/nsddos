...
from nsddos.runtime.freshness.models import (
    FreshnessEvaluation,
    FreshnessStatus,
    RuntimeFreshness,
    ValidityState,
...
def _age_seconds(observed_at: str, reference_at: str) -> float:
    return max(
        0.0, (_parse_iso(reference_at) - _parse_iso(observed_at)).total_seconds()
    )


def _validity(
    age_seconds: float, stale_after: int, max_age: int, replay_only: bool
) -> ValidityState:
    if replay_only:
...
def evaluate_freshness(
    scope: str, payload: dict[str, Any], reference_at: str | None = None
) -> FreshnessEvaluation:
    start = monotonic()
    window = get_window(scope)
    created_at = normalize_time(str(payload.get("created_at", "")))
    observed_at = normalize_time(
        str(payload.get("observed_at", payload.get("timestamp", "")))
    )
    ...
    validity = _validity(
        age_seconds, window.stale_after_seconds, window.max_age_seconds, replay_only
    )
    ...
    freshness_status = (
        ...
        if final_validity == ValidityState.REPLAY_ONLY
            ...
            FreshnessStatus.DEGRADED_LIVE
            if final_validity
            in {
                ValidityState.STALE,
                ValidityState.EXPIRED,
                ValidityState.INCONSISTENT,
                ValidityState.DIVERGENT,
    ...
    evaluation = FreshnessEvaluation(
        freshness=RuntimeFreshness(
            ...
            replay_validity=(
                "replay-safe"
                if replay_only or bool(payload.get("replay_safe", True))
                else "non-replay-safe"
            ),
