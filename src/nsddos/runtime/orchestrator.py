...
from nsddos.runtime.models import RuntimeExecutionState, RuntimePhaseResult, RuntimePipelineSnapshot
...
def _phase_result(phase: str, status: str, detail: str, started: str, start_time: float, artifacts: dict[str, Any] | None = None) -> RuntimePhaseResult:
...
def save_pipeline_snapshot(plan: dict[str, Any], state: dict[str, Any], graph: dict[str, Any]) -> Path:
    """Persist pipeline snapshot."""
    PIPELINE_DIR.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    snapshot = RuntimePipelineSnapshot(timestamp=_now(), plan=plan, state=state, execution_graph=graph)
    path = PIPELINE_DIR / f"pipeline-{stamp}.json"
    atomic_write_json(path, snapshot.to_dict())
    return path


def _phase_handlers(config: dict[str, Any], preset: str) -> dict[str, Callable[[], tuple[str, str, dict[str, Any]]]]:
    ...
    return {
        ...
        "runtime_prepare": lambda: ("pass" if _runtime_files_exist() else "warn", "canonical runtime files checked", {}),
        "providers_prepare": lambda: ("pass", "providers resolved from preset", {"preset": load_preset(preset)}),
        "docker_runtime_start": lambda: _docker_phase(config),
        "controller_start": lambda: ("warn", "controller start delegated to canonical runtime; skipped in degraded dry run", {}),
        "telemetry_start": lambda: ("warn", "telemetry start delegated to canonical runtime; skipped in degraded dry run", {}),
        "topology_start": lambda: ("warn", "topology start requires Linux OVS/Mininet runtime", {}),
...
def _verify_phase(config: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
    ...
    return status, f"failed={failed} total={len(results)}", {"results": [item.to_dict() for item in results]}


def _verification_prepare_phase(config: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
...
def _verification_evidence_phase(config: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
    from nsddos.runtime.verification.engine import explain_verification

    explanation = explain_verification(config)
    evidence = explanation.get("evidence", [])
    return "pass" if evidence else "warn", f"evidence={len(evidence)}", {"evidence": evidence}


def _verification_finalize_phase(config: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
...
def _api_validate_phase(config: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
    ...
    return "pass" if ok else "warn", "API readonly/query-backed contract checked", explanation
...
def _api_finalize_phase(config: dict[str, Any]) -> tuple[str, str, dict[str, Any]]:
    ...
    summary = get_route_summary()
    return "pass", "API integrity finalized", {"routes": summary.get("endpoint_count", 0)}
...
def execute_pipeline(config: dict[str, Any], preset: str = "minimal-lab", mode: str = "bootstrap") -> RuntimeExecutionState:
    ...
    state = RuntimeExecutionState(plan=plan.name, status="running", started_at=_now(), preset=preset, profile=plan.profile)
    handlers = _phase_handlers(config, preset)
    emit_runtime_event(f"pipeline.{mode}", "started", "Canonical runtime pipeline started.", {"preset": preset})
    ...
    for phase in plan.phases:
        ...
        result = _phase_result(phase.name, status, detail, started, start_time, artifacts)
        state.results.append(result)
        emit_runtime_event(f"pipeline.phase.{phase.name}", status, detail, {"preset": preset})

    state.current_phase = None
    state.completed_at = _now()
    state.status = "degraded_pipeline" if any(item.status == "warn" for item in state.results) else "stable_pipeline"
    ...
    emit_runtime_event(f"pipeline.{mode}", "completed", "Canonical runtime pipeline completed.", {"status": state.status, "snapshot": str(snapshot)})
    return state


def shutdown_pipeline(config: dict[str, Any], preset: str = "minimal-lab") -> RuntimeExecutionState:
    """Run deterministic shutdown pipeline."""
    state = RuntimeExecutionState(plan="canonical-shutdown", status="running", started_at=_now(), preset=preset)
    phases = ["evidence_capture", "topology_stop", "telemetry_stop", "controller_stop", "docker_runtime_stop", "state_finalize"]
    emit_runtime_event("pipeline.shutdown", "started", "Canonical shutdown pipeline started.", {"preset": preset})
    for phase in phases:
        started = _now()
        start_time = monotonic()
        ...
        if phase == "evidence_capture":
            ...
            artifacts = {}
        state.results.append(_phase_result(phase, status, detail, started, start_time, artifacts))
        emit_runtime_event(f"pipeline.phase.{phase}", status, detail, {"preset": preset})
    state.completed_at = _now()
    state.status = "stable_pipeline"
    graph = build_execution_graph(config, preset=preset)
    save_pipeline_snapshot({"name": state.plan}, state.to_dict(), graph)
    emit_runtime_event("pipeline.shutdown", "completed", "Canonical shutdown pipeline completed.", {"status": state.status})
    return state
...
def use_runtime_preset(config: dict[str, Any], preset: str) -> dict[str, Any]:
    ...
    write_runtime_state(runtime_state)
    save_pipeline_snapshot(plan.to_dict(), {"preset": runtime_state.preset_state}, graph)
    emit_runtime_event("pipeline.preset", "configured", "Runtime preset configured.", {"preset": preset})
