...
def build_block_action(action_type: str, target_ip: str, reason: str) -> MitigationActionPayload:
    duration = 86400 if action_type == "permanent_ban" else 900 if action_type == "temporary_ban" else 3600
...
def block_evidence(action: MitigationActionPayload) -> str:
    return hashlib.sha256(to_canonical_json(action.to_dict()).encode("utf-8")).hexdigest()
