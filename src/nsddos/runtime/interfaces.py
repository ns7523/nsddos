...
def correlate_interfaces(config: dict[str, Any]) -> InterfaceCorrelation:
    ...
    for ovs_name, link_name in _expected_switch_interfaces(metadata):
        record = InterfaceRecord(
            ...
            switch_id=identity_map.switches[0].canonical_id if identity_map.switches else None,
            ovs_name=ovs_name,
            mininet_link=link_name,
            sflow_name=next((name for name in sflow_ifaces if ovs_name in name or link_name in name), None),
            controller_port=None,
            visible_in_ovs=ovs_name in ovs_bridges.get("s1", type("B", (), {"interfaces": []})()).interfaces,
            visible_in_sflow=any(ovs_name in name or link_name in name for name in sflow_ifaces),
    ...
    live_ovs_names = set(ovs_bridges.get("s1", type("B", (), {"interfaces": []})()).interfaces)
    ...
    stability = "unstable" if history.get("topology_changed") else ("stable" if not missing and not duplicates and not orphan else "partial")
