...
def _namespace_shell(
    host: str, script: str, timeout: int = 30
) -> subprocess.CompletedProcess[str]:
...
def _start_victim_service(
    victim: str, target_ip: str, target_port: int
) -> threading.Thread:
    _namespace_shell(
        victim,
...
def _build_attack_script(
    attack_type: str, target_ip: str, target_port: int, duration_seconds: int
) -> str:
...
def _pick_protocol(flow: dict[str, Any], target_port: int) -> str:
    ...
    port = int(
        _pick_number(
            flow, "destination_port", "dst_port", "destinationPort", "port", "dstport"
        )
    )
    ...
    for index, flow in enumerate(raw_flows, start=1):
        ...
        if protocol == "http" and duration <= 0 and attack_type == "slowloris":
            duration = max(
                ...
                - datetime.fromisoformat(
                    attack_started_at.replace("Z", "+00:00")
                ).timestamp(),
            )
        metadata = {
        ...
        events.append(
            StreamEvent(
                ...
                freshness_state=(
                    "valid" if snapshot.health_state == "healthy" else "degraded"
                ),
                destination_port=destination_port,
                duration_seconds=duration,
                flags=str(flow.get("tcpFlags", flow.get("flags", ""))),
    ...
    return (
        StreamEvent(
            ...
            metadata={
                ...
                "http_rate": (
                    max(snapshot.packet_rate, 1.0)
                    if attack_type in {"http_flood", "slowloris"}
                    else 0.0
                ),
                "partial_connection_rate": (
                    max(snapshot.connection_rate, 1.0)
                    if attack_type == "slowloris"
                    else 0.0
                ),
            },
...
def _collect_live_inputs(
    config: dict[str, Any],
    attack_type: str,
    target_ip: str,
    target_port: int,
    attack_started_at: str,
) -> tuple[Any, list[dict[str, Any]], tuple[StreamEvent, ...]]:
    registry = build_live_provider_registry(config)
    snapshot = collect_live_telemetry(config)
    flows_result = registry.pool.get_json(
        f"{registry.sflowrt.api_url}/flows/json?maxFlows=20&timeout=1"
    )
    raw_flows = flows_result.payload if isinstance(flows_result.payload, list) else []
    events = _build_stream_events(
        raw_flows, snapshot, attack_type, target_ip, target_port, attack_started_at
    )
...
def _record_false_positives(bucket: dict[str, int], evaluation: Any) -> None:
    ...
    if detection.get("attack_detected"):
        bucket["detection"] += 1
    if (
        ml.get("predicted_attack_type") not in {"", "normal"}
        and float(ml.get("attack_probability", 0.0)) >= 0.5
    ):
        bucket["ml"] += 1
...
def _attack_seen(
    ...
    for flow in raw_flows:
        source = str(
            flow.get("source") or flow.get("src_ip") or flow.get("source_ip") or ""
        )
...
def _summarize_scenario(
    ...
    metrics = {
        "detection_latency_seconds": _latency_seconds(
            attack_started_at,
            (
                first_sflow_seen_at
                if not detection_payload
                else detection_payload.get("telemetry_timestamp")
            ),
        ),
        "mitigation_latency_seconds": _latency_seconds(
            attack_started_at,
            mitigation_payload.get("timestamp") or mitigation_payload.get("created_at"),
        ),
        "dashboard_latency_seconds": _latency_seconds(
            attack_started_at, first_dashboard_visible_at
        ),
        ...
        "false_positives": {
            ...
            "aggregate_rate": (
                sum(false_positives["warmup"].values())
                + sum(false_positives["cooldown"].values())
            )
            / max(len(probes), 1),
        },
    }
    return {
        ...
        "subsystems": {
            ...
            "ml_classified": ml_payload.get("predicted_attack_type") == attack_type
            or bool(ml_payload.get("attack_probability", 0.0) >= 0.5),
            "policy_reacted": policy_payload.get("recommended_action")
            not in {"", "alert_only"},
            "mitigation_executed": mitigation_payload.get("mitigation_status")
            in {"enforced", "verified"},
    ...
    try:
        ...
        for _ in range(max(warmup, 1)):
            if stop_event is not None and stop_event.is_set():
                break
            snapshot, raw_flows, events = _collect_live_inputs(
                config, attack_type, target_ip, target_port, warmup_started_at
            )
            dropped_before = int(snapshot.dropped_packets)
            latest_evaluation = process_stream_events(
                config, session_id=f"attack-{attack_type}-warmup", events=events
            )
        ...
        attack_script = _build_attack_script(
            attack_type, target_ip, target_port, attack_seconds
        )
        ...
        while time.time() < deadline:
            if stop_event is not None and stop_event.is_set():
                ...
                break
            snapshot, raw_flows, events = _collect_live_inputs(
                config, attack_type, target_ip, target_port, attack_started_at
            )
            if first_sflow_seen_at is None and (
                _attack_seen(
                    raw_flows, attack_type, attacker_ip, target_ip, target_port
                )
                or snapshot.active_flows > 0
            ):
                first_sflow_seen_at = _now()
            latest_evaluation = process_stream_events(
                config, session_id=f"attack-{attack_type}-live", events=events
            )
        ...
        for _ in range(max(cooldown, 1)):
            ...
            latest_evaluation = process_stream_events(
                config, session_id=f"attack-{attack_type}-cooldown", events=events
            )
            _record_false_positives(cooldown_false_positives, latest_evaluation)
            probe_samples.append(_probe_http(probe, target_ip, target_port))
            time.sleep(1)
        protocol_mix = _count_by_protocol(
            raw_flows if "raw_flows" in locals() else [], target_port
        )
        ...
        return {
            ...
            "first_detection_at": (
                latest_evaluation.detection_payload.get("telemetry_timestamp")
                if latest_evaluation
                else None
            ),
            "first_ml_classification_at": (
                latest_evaluation.ml_payload.get("timestamp")
                if latest_evaluation
                else None
            ),
            "first_policy_action_at": (
                latest_evaluation.policy_payload.get("timestamp")
                if latest_evaluation
                else None
            ),
            "first_mitigation_enforced_at": (
                latest_evaluation.mitigation_payload.get("timestamp")
                if latest_evaluation
                else None
            ),
            ...
            "root_cause": (
                ""
                if all(summary["subsystems"].values())
                else "subsystem_transition_missing"
            ),
            "fix_applied": "",
        }
    finally:
    ...
    *,
    ...
    suite = {
        ...
        "environment": {
            ...
            "simulation_source_enabled": runtime_config["runtime"]["simulation"][
                "source_enabled"
            ],
