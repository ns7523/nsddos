...
def apply_filters(
    items: list[dict[str, Any]], filters: tuple[RuntimeQueryFilter, ...]
) -> list[dict[str, Any]]:
    ...
    for query_filter in filters:
        if query_filter.operator == "eq":
            result = [
                item
                for item in result
                if _value(item, query_filter.field) == query_filter.value
            ]
        elif query_filter.operator == "contains":
            result = [
                item
                for item in result
                if str(query_filter.value) in str(_value(item, query_filter.field))
            ]
        elif query_filter.operator == "exists":
            result = [
                item for item in result if _value(item, query_filter.field) is not None
            ]
        else:
