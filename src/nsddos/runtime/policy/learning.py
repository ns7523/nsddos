...
def load_learning_state(*, lock_scope: TextIOWrapper | None = None) -> PolicyLearningState:
    payload = recover_json(LEARNING_PATH, {}, lock_scope=lock_scope)
    return PolicyLearningState(
        attack_signature_counts={str(k): int(v) for k, v in (payload.get("attack_signature_counts", {}) or {}).items()},
        source_ip_counts={str(k): int(v) for k, v in (payload.get("source_ip_counts", {}) or {}).items()},
        subnet_counts={str(k): int(v) for k, v in (payload.get("subnet_counts", {}) or {}).items()},
        mitigation_success_rate={str(k): float(v) for k, v in (payload.get("mitigation_success_rate", {}) or {}).items()},
    )


def save_learning_state(state: PolicyLearningState, *, lock_scope: TextIOWrapper | None = None) -> None:
