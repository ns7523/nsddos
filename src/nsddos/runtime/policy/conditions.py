...
def evaluate_conditions(
    *,
    ...
    repeated_attack_frequency = (
        len(
            [
                item
                for item in history
                if item.attack_type == attack_type and item.source_ip == source_ip
            ]
        )
        + 1
    )
    repeated_source_ip = any(item.source_ip == source_ip for item in history)
    repeated_subnet_attacks = bool(source_subnet) and any(
        item.source_subnet == source_subnet for item in history
    )
