...
def baseline_rule(detection: DetectionEvaluation) -> PolicyRule:
    if not detection.attack_detected or detection.confidence_score < 0.5:
        return PolicyRule("policy_alert_only", detection.attack_type, "alert_only", "normal_or_low_confidence")
    mapping = {
        ...
        "slowloris": ("policy_connection_reset", "connection_reset", "baseline_slowloris_response"),
        "connection_exhaustion": ("policy_quarantine_host", "quarantine_host", "baseline_connection_quarantine"),
    }
