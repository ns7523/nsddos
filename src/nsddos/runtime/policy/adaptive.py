...
from nsddos.runtime.policy.contracts_models import PolicyConditionResult, PolicyThresholdState
...
def adaptive_action(
    ...
    *,
    ...
    if conditions.replay_restricted or conditions.freshness_degraded or not conditions.confidence_threshold_met:
        return "alert_only", 0
    ...
    if conditions.repeated_source_ip and threshold_state.threshold_score >= 0.35:
        return "block_ip", 2
    if (
        conditions.repeated_subnet_attacks
        and threshold_state.threshold_score >= 0.55
    ):
