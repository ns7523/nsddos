...
from nsddos.runtime.models import FlowState, RuntimeCollectionBundle, TelemetryFreshness, TelemetryState
from nsddos.runtime.performance import timed
from nsddos.runtime.profiles import detect_runtime_profile
from nsddos.runtime.providers.live.telemetry import collect_live_telemetry, live_snapshot_to_collection_state
from nsddos.runtime.simulation import contract_to_collection_state, generate_attack_traffic
...
def collect_runtime_state(config: dict[str, Any]) -> RuntimeCollectionBundle:
    ...
    simulation_source_enabled = bool(config.get("runtime", {}).get("simulation", {}).get("source_enabled", False))
    if live_enabled:
        live_snapshot = timed("live_provider_collection_ms", timings, lambda: collect_live_telemetry(config))
        live_state = timed("live_collection_state_ms", timings, lambda: live_snapshot_to_collection_state(live_snapshot))
        ...
        simulation_contract = timed("simulation_collection_ms", timings, lambda: generate_attack_traffic(config))
        simulation_state = timed("simulation_collection_state_ms", timings, lambda: contract_to_collection_state(simulation_contract))
        ...
        registry = timed("provider_registry_ms", timings, lambda: runtime_registry(config))
        provider_status = timed("provider_status_ms", timings, lambda: collect_provider_status_from_registry(registry))
        flows = timed("flow_collection_ms", timings, lambda: sample_flow_visibility(config, interval=1.0))
        freshness = timed("freshness_collection_ms", timings, lambda: telemetry_freshness(config, interval=1.0))
        telemetry = timed(
            "telemetry_normalization_ms",
            timings,
            lambda: build_telemetry_state_from_status(provider_status.get("sflowrt", {}), flows, freshness),
        )
    controller = timed("controller_collection_ms", timings, lambda: normalize_controller_topology(config))
    profile = timed("profile_detection_ms", timings, detect_runtime_profile)
    capabilities = timed("capability_detection_ms", timings, detect_runtime_capabilities)
    environment = timed("environment_validation_ms", timings, lambda: validate_runtime_environment(config))
    reproducibility = timed("reproducibility_analysis_ms", timings, lambda: analyze_reproducibility(config))
