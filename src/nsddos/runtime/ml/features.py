...
def extract_ml_features(
    ...
    if not isinstance(flows, list):
        flows = []
    packet_values = [
        _numeric(flow, "packets") for flow in flows if isinstance(flow, dict)
    ]
    ...
    for flow in flows:
        ...
        protocol_volume[protocol] = protocol_volume.get(protocol, 0.0) + _numeric(
            flow, "packets"
        )
    ...
    packet_variance = (
        sum((value - mean_packets) ** 2 for value in packet_values)
        / max(len(packet_values), 1)
        if packet_values
    ...
    flow_duration = (
        ...
        if durations
        else detection.feature_vector.flow_duration
    )
