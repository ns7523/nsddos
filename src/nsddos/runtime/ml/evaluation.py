...
def compute_evaluation_metrics(
    dataset: MLDatasetSnapshot, feedback: MLFeedbackState
) -> MLEvaluationMetrics:
    rows = dataset.rows
    if not rows:
    ...
    successful_attacks = sum(
        1 for row in rows if row.attack_label and row.mitigation_success
    )
    ...
    confidence_quality = max(
        ...
        min(
            1.0,
            (precision * 0.4) + (recall * 0.4) + ((1.0 - false_positive_rate) * 0.2),
        ),
    )
