...
from nsddos.runtime.streaming.contracts import StreamBufferState, StreamCheckpoint, StreamEvent, StreamQueueState, StreamSession
...
def _event_from_dict(payload: dict[str, object]) -> StreamEvent:
    return StreamEvent(
        ...
        timestamp=datetime.fromisoformat(str(payload.get("timestamp", "2100-01-01T00:00:00+00:00")).replace("Z", "+00:00")),
...
def restore_checkpoint() -> StreamCheckpoint | None:
    ...
    if not payload:
        return None
    queue_events = tuple(_event_from_dict(item) for item in payload.get("queue_state", {}).get("events", []))
    buffer_events = tuple(_event_from_dict(item) for item in payload.get("buffer_state", {}).get("events", []))
    queue_state = StreamQueueState(
        events=queue_events,
        queue_depth=int(payload.get("queue_state", {}).get("queue_depth", len(queue_events))),
        enqueued_count=int(payload.get("queue_state", {}).get("enqueued_count", len(queue_events))),
        dequeued_count=int(payload.get("queue_state", {}).get("dequeued_count", 0)),
    )
    buffer_state = StreamBufferState(
        events=buffer_events,
        max_size=int(payload.get("buffer_state", {}).get("max_size", len(buffer_events))),
...
def restore_session() -> StreamSession | None:
    ...
    return StreamSession(
        ...
        session_start=datetime.fromisoformat(str(payload.get("session_start", "2100-01-01T00:00:00+00:00")).replace("Z", "+00:00")),
...
def validate_recovery_state(checkpoint: StreamCheckpoint | None, session: StreamSession | None) -> list[str]:
