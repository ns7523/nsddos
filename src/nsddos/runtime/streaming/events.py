...
def _live_events(config: dict[str, Any]) -> tuple[StreamEvent, ...]:
    ...
    target_ip = (
        snapshot.topology_state.hosts[0]
        if snapshot.topology_state.hosts
        else "10.0.0.1"
    )
    ...
    if not source_points:
        source_points = (
            type(
                "Point", (), {"key": "10.0.0.10", "value": snapshot.packet_rate or 1.0}
            )(),
        )
    ...
    for index, point in enumerate(ordered, start=1):
        ...
        protocol = "tcp"
        if (
            snapshot.syn_rate >= snapshot.udp_rate
            and snapshot.syn_rate >= snapshot.icmp_rate
        ):
            protocol = "tcp"
        ...
        events.append(
            StreamEvent(
                event_id=deterministic_id(
                    "stream-event", f"live:{point.key}:{index}:{timestamp.isoformat()}"
                ),
                source_type="live",
                packet_rate=packet_rate,
                byte_rate=max(
                    snapshot.byte_rate / max(len(ordered), 1), packet_rate * 64.0
                ),
                connection_rate=max(
                    snapshot.connection_rate / max(len(ordered), 1), 1.0
                ),
                protocol=protocol,
                source_ip=point.key,
...
def _simulation_events(config: dict[str, Any]) -> tuple[StreamEvent, ...]:
    ...
    for index, packet in enumerate(contract.packet_metadata, start=1):
        ...
        if schedule.emit_at_ms:
            timestamp = base_time.fromtimestamp(
                base_time.timestamp() + (schedule.emit_at_ms / 1000.0),
                tz=base_time.tzinfo,
            )
        events.append(
            StreamEvent(
                event_id=deterministic_id(
                    "stream-event",
                    f"simulation:{packet.sequence_id}:{timestamp.isoformat()}",
                ),
...
def _collection_events() -> tuple[StreamEvent, ...]:
    ...
    observed_at = payload.get("flow_state", {}).get("observed_at") or payload.get(
        "telemetry_state", {}
    ).get("last_flow_timestamp")
    timestamp = (
    ...
    return (
        StreamEvent(
            event_id=deterministic_id(
                "stream-event", f"collection:{timestamp.isoformat()}:{flow_count}"
            ),
