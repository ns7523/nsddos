...
def build_window_state(
    ...
    *,
    ...
    if not events:
        return StreamWindowState(window_kind=window_kind, window_seconds=window_seconds, windows=(), active_events=0)
    ...
    if window_kind == "tumbling":
        ...
        for item in ordered:
            if (item.timestamp - current_start).total_seconds() >= window_seconds and current:
                start = current[0].timestamp
                end = current[-1].timestamp
                windows.append(
                    StreamWindow(
                        window_id=deterministic_id("stream-window", f"tumbling:{start.isoformat()}:{len(current)}"),
        ...
        return StreamWindowState(window_kind=window_kind, window_seconds=window_seconds, windows=tuple(windows), active_events=len(ordered))
    if window_kind == "fixed_time":
        ...
        for index, item in enumerate(ordered, start=1):
            ...
            windows.append(
                StreamWindow(
                    window_id=deterministic_id("stream-window", f"fixed:{index}:{anchor.isoformat()}"),
    ...
    for index, item in enumerate(ordered, start=1):
        ...
        windows.append(
            StreamWindow(
                window_id=deterministic_id("stream-window", f"sliding:{index}:{item.timestamp.isoformat()}"),
                start_timestamp=start.isoformat(),
                end_timestamp=item.timestamp.isoformat(),
                events=tuple(candidate for candidate in ordered if start <= candidate.timestamp <= item.timestamp),
            )
        )
    return StreamWindowState(window_kind=window_kind, window_seconds=window_seconds, windows=tuple(windows), active_events=len(ordered))
