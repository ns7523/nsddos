...
from nsddos.runtime.streaming.dispatcher import dispatch_detection, dispatch_mitigation, dispatch_ml, dispatch_policy
...
from nsddos.runtime.streaming.recovery import restore_checkpoint, restore_session, validate_recovery_state
...
from nsddos.runtime.streaming.validation import validate_stream_events, validate_streaming_evaluation
...
def _persist_evaluation(evaluation: StreamingEvaluation) -> None:
    ...
    with locked_persistence_scope(STREAMING_DIR) as lock_scope:
        atomic_write_json(STREAMING_DIR / f"streaming-{stamp}.json", payload, lock_scope=lock_scope)
...
def process_stream_events(
    ...
    source_mode, source_events = resolve_source_events(config) if events is None else ("manual", events)
    ...
    session = build_session(
        ...
        session_id=session_id or (restored_session.session_id if restored_session is not None else None),
        processed_events_count=len(buffer_state.events),
        last_sequence_number=buffer_state.events[-1].sequence_number if buffer_state.events else 0,
        session_state="throttled" if backpressure.throttled else "active",
        session_start=restored_session.session_start if restored_session is not None and session_id is None else None,
    )
    ...
    throughput = float(len(buffer_state.events)) / max(float(settings.get("window_seconds", 10)), 1.0)
