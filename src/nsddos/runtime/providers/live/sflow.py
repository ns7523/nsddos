...
def collect_sflow_telemetry(
    ...
    packet_rate = sum(_number(item.get("packets", item.get("frames", 0))) for item in flows)
    byte_rate = sum(_number(item.get("bytes", item.get("octets", 0))) for item in flows)
    connection_rate = sum(_number(item.get("connections", item.get("flows", 0))) for item in flows)
    ...
    source_counter = Counter(str(item.get("source", item.get("src_ip", "unknown"))) for item in flows if item.get("source") or item.get("src_ip"))
    port_counter = Counter(str(item.get("destination_port", item.get("dst_port", 0))) for item in flows if item.get("destination_port") or item.get("dst_port"))
    topology_interfaces = {
        str(item.get("ifname", ""))
        for item in flows
        if item.get("ifname")
    ...
    if not topology_interfaces and helper_links:
        topology_interfaces = {name for name in helper_links.values() if name.startswith("s1-eth")}
    return {
        "reachable": provider.is_reachable(),
        "latency_ms": max(flows_result.latency_ms, metrics_result.latency_ms, topology_result.latency_ms),
        ...
        "source_ip_distribution": tuple(DistributionPoint(key=key, value=float(value)) for key, value in sorted(source_counter.items())),
        "destination_port_distribution": tuple(DistributionPoint(key=key, value=float(value)) for key, value in sorted(port_counter.items())),
