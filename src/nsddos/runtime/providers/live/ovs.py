...
def collect_ovs_telemetry(provider: OVSProvider) -> dict[str, object]:
    ...
    interfaces = tuple(sorted(name for item in state.bridges for name in item.interfaces))
    ...
    return {
        ...
        "active_connection_state": state.service_running and any(item.controller_connected for item in state.bridges),
