...
def build_install_requirements(
    ...
    if not scan.docker.installed:
        requirements.append(
            InstallRequirement("A", "Install Docker", "Docker engine required for NSDDOS runtime.")
        )
    if not scan.docker_compose.installed:
        requirements.append(
            InstallRequirement("B", "Install Docker Compose", "Compose required for multi-service runtime orchestration.")
        )
    if scan.docker.installed and not scan.docker_daemon_running:
        requirements.append(
            InstallRequirement("C", "Start Docker Daemon", "Docker CLI present but daemon not running.")
        )
    if not scan.git.installed:
        requirements.append(
            InstallRequirement("D", "Install Git", "Git required for source sync and developer workflows.")
        )
    if not scan.virtualenv_active:
        requirements.append(
            InstallRequirement("E", "Create Virtual Environment", "Create isolated Python environment before runtime setup.")
        )
    if not scan.docker_permissions_ready:
        requirements.append(
            InstallRequirement("F", "Configure Docker Permissions", "Grant current user Docker runtime access.")
        )
