...
def build_replication_state(
    ...
    target_nodes = sorted({partition.node_id for partition in partitions} | {node_id for partition in partitions for node_id in partition.replica_node_ids})
    lag_ms = float(len(partitions) * replication_factor * 3)
    return ReplicationState(
        replication_factor=replication_factor,
        replicated_resources=("policy_state", "ml_model_state", "stream_checkpoints", "runtime_configuration"),
