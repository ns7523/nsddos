...
def _format_uptime(started_at: str) -> str:
    ...
    if started is None:
        return "--:--"
    elapsed = max(int((datetime.now(timezone.utc) - started.astimezone(timezone.utc)).total_seconds()), 0)
...
def _table(title: str, columns: tuple[str, ...], rows: tuple[tuple[str, ...], ...], empty_message: str = "NO DATA") -> UiTableSection:
    return UiTableSection(
        title=title,
        columns=tuple(UiTableColumn(key=f"c{index}", label=label) for index, label in enumerate(columns)),
        rows=tuple(UiTableRow(values=row) for row in rows),
        empty_message=empty_message,
    )


def _bundle_table(title: str, rows: tuple[tuple[str, str], ...], empty_message: str = "NO DATA") -> UiTableSection:
    return _table(title, ("FIELD", "VALUE"), rows, empty_message)


def _event_entry(timestamp: str, severity: str, detail: str, source: str) -> UiEventFeedEntry:
    parsed = _parse_timestamp(timestamp)
    stamp = parsed.astimezone(timezone.utc).strftime("%H:%M:%S") if parsed is not None else timestamp
...
def _pair_points(items: list[list[Any]] | tuple[tuple[Any, Any], ...], *, upper: bool = False) -> tuple[UiChartPoint, ...]:
    points: list[UiChartPoint] = []
    for item in list(items):
        if not isinstance(item, (list, tuple)) or len(item) < 2:
            continue
        label = str(item[0]).replace("_", " ").upper() if upper else str(item[0]).replace("_", " ").title()
...
def _build_attack_chart(dashboard: dict[str, Any]) -> UiChartModel:
    counts = {str(name): _safe_float(value, 0.0) for name, value in dashboard.get("attacks", {}).get("attack_types", [])}
    return UiChartModel(
        chart_id="attack-distribution",
        title="ATTACK DISTRIBUTION",
        chart_type="bar",
        unit="EVENTS",
        points=tuple(UiChartPoint(label=label, value=counts.get(key, 0.0)) for key, label in ATTACK_ORDER),
    )


def _build_traffic_chart(dashboard: dict[str, Any], live: dict[str, Any], title: str = "LIVE PACKETS / SEC") -> UiChartModel:
    visualizations = dashboard.get("visualizations", [])
    points = _pair_points(visualizations[0].get("points", []), upper=True) if visualizations else ()
    if not points:
        base = _safe_float(dashboard.get("metrics", {}).get("packet_throughput", live.get("packet_rate", 0.0)), 0.0)
        points = (
...
def _merged_feed(history: dict[str, Any], *, limit: int | None = None) -> tuple[UiEventFeedEntry, ...]:
        ...
        for item in history.get(source_key, []):
            items.append(
                (
                    ...
                    _safe_text(item.get("detail", item.get("message", item.get("event_type", "event"))), "event"),
                    source_name,
                )
            )
    ...
    if limit is not None:
        items = items[:limit]
    return tuple(_event_entry(timestamp, severity, detail, source) for timestamp, severity, detail, source in items)
...
def _system_status(checks: dict[str, Any]) -> str:
    ...
    containers_ok = bool(checks.get("containers", docker_ok and floodlight_ok and sflowrt_ok))
...
def _status_bar(bundle: dict[str, Any]) -> UiStatusBarSnapshot:
    ...
    return UiStatusBarSnapshot(
        fields=(
            ...
            UiStatusField("THREAT LEVEL", _safe_text(detection.get("risk_level", "LOW")).upper(), _field_state(_safe_text(detection.get("risk_level", "LOW")).upper())),
            UiStatusField("UPTIME", _format_uptime(startup.started_at) if startup is not None else "--:--"),
            UiStatusField("PACKETS/S", _fmt_packets(dashboard.get("metrics", {}).get("packet_throughput", live.get("packet_rate", 0.0)))),
            UiStatusField("ACTIVE ATTACKS", _fmt_int(dashboard.get("active_attacks", 0)), "bad" if int(_safe_float(dashboard.get("active_attacks", 0), 0.0)) else "good"),
        ),
        live_state="live",
    )
...
def _service_rows(bundle: dict[str, Any]) -> tuple[UiServiceRow, ...]:
    ...
    services = (
        ...
        ("DETECTOR", detection.get("attack_type") is not None, _safe_text(detection.get("attack_type", "ready")).upper()),
        ("MITIGATION", mitigation.get("execution_result") is not None, _safe_text(mitigation.get("execution_result", "idle")).upper()),
...
def _topology(bundle: dict[str, Any]) -> UiTopologyMap:
    ...
    service_states = {
        "internet": "ONLINE",
        "router": "ONLINE" if (checks.get("docker_daemon") or checks.get("docker")) else "OFFLINE",
        ...
        "detection": "ONLINE" if bundle["detection"].get("attack_type") is not None else "OFFLINE",
        "mitigation": "ONLINE" if bundle["mitigation"].get("execution_result") is not None else "DEGRADED",
    }
    nodes = tuple(
        UiTopologyNode(
            node_id=node_id,
...
class UiPageBuilder:
    ...
    def _bundle(self) -> dict[str, Any]:
        ...
        return {
            ...
            "provider_health": self._api("/runtime/provider-health", {"limit": 25, "offset": 0})["payload"],
            "service": self._api("/runtime/service", {"limit": 25, "offset": 0})["payload"],
    ...
    def overview(self) -> UiPagePayload:
        ...
        return UiPagePayload(
            ...
            feed=UiEventFeed(title="LIVE EVENT FEED", entries=_merged_feed(bundle["history"], limit=5), preview=True),
            services=_service_rows(bundle),
            tables=(
                _bundle_table(
                    "RUNTIME",
                    (
                        ("BANDWIDTH", _fmt_bandwidth(bundle["dashboard"].get("metrics", {}).get("byte_throughput", bundle["live"].get("byte_rate", 0.0)))),
                        ("ACTIVE FLOWS", _fmt_int(bundle["live"].get("active_flows", 0))),
                        ("SESSIONS", _fmt_int(len(bundle["service_diagnostics"].get("active_sessions", [])))),
                        ("UPDATED", _safe_text(bundle["live"].get("timestamp", bundle["dashboard"].get("timestamp", _now_iso())))),
                    ),
                ),
            ),
            updated_at=_safe_text(bundle["live"].get("timestamp", bundle["dashboard"].get("timestamp", _now_iso()))),
        )
    ...
    def infrastructure(self) -> UiPagePayload:
        ...
        provider_rows = tuple(
            (
                _safe_text(item.get("provider_name", item.get("id", "provider"))).upper(),
                _safe_text(item.get("health_state", item.get("state", "unknown"))).upper(),
                _safe_text(item.get("detail", item.get("controller_status", "")), "UNKNOWN").upper(),
            )
        ...
        return UiPagePayload(
            ...
            tables=(
                ...
                _bundle_table(
                    "HOST CHECKS",
                    (
                        ("DOCKER", "ONLINE" if bundle["health"].get("checks", {}).get("docker_daemon") else "OFFLINE"),
                        ("SESSION", "PRESENT" if bundle["startup_session"] is not None else "MISSING"),
                    ),
                ),
            ),
            updated_at=_now_iso(),
        ...
        return UiPagePayload(
            ...
            charts=(
                UiChartModel(
                    ...
                    unit="SCORE",
                    points=(
                        UiChartPoint("CONFIDENCE", _safe_float(detection.get("confidence", 0.0))),
                        UiChartPoint("ML CLASS", _safe_float(ml.get("confidence_score", 0.0))),
                        UiChartPoint("ANOMALY", _safe_float(ml.get("anomaly_score", 0.0))),
            ...
            tables=(
                _bundle_table(
                    "DETECTION STATE",
                    (
                        ("ATTACK TYPE", _safe_text(detection.get("attack_type", "normal")).upper()),
            ...
            feed=UiEventFeed(title="DETECTION EVENTS", entries=_merged_feed(bundle["history"], limit=8)),
        ...
        return UiPagePayload(
            ...
            tables=(
                _bundle_table(
                    "ACTIVE MITIGATION",
                    (
                        ("REQUIRED", "YES" if mitigation.get("mitigation_required") else "NO"),
            ...
            feed=UiEventFeed(title="MITIGATION FEED", entries=_merged_feed(bundle["history"], limit=10)),
            updated_at=_safe_text(bundle["live"].get("timestamp", _now_iso())),
        )
        ...
        return UiPagePayload(
            ...
            charts=(
                UiChartModel(
                    ...
                    chart_type="bar",
                    unit="COUNT",
                    points=_pair_points(dashboard.get("attacks", {}).get("source_ips", []), upper=True),
                ),
                UiChartModel(
                    ...
                    chart_type="bar",
                    unit="RATIO",
                    points=_pair_points(dashboard.get("threat_intel", {}).get("suspicious_protocol_concentration", []), upper=True),
                ),
            ),
            tables=(
                _bundle_table(
                    "TELEMETRY",
                    (
                        ("PACKET RATE", _fmt_packets(bundle["live"].get("packet_rate", 0.0))),
            ...
            feed=UiEventFeed(title="TRAFFIC FEED", entries=_merged_feed(bundle["history"], limit=10)),
            updated_at=_safe_text(bundle["live"].get("timestamp", _now_iso())),
        )
            ...
            lab_console=UiLabConsolePayload(
                ...
                + (
                    UiLabNode(
                        ...
                        detail=_safe_text(detection.get("attack_type", "normal")).upper(),
                        metadata={"risk": _safe_text(detection.get("risk_level", "low")).upper()},
                        actions=("inspect",),
                    ),
                    UiLabNode(
                        node_id="mitigation",
                        ...
                        kind="service",
                        state="active" if mitigation.get("mitigation_required") else "planned",
                        detail=_safe_text(mitigation.get("mitigation_action", "alert_only")).replace("_", " ").upper(),
                        metadata={"result": _safe_text(mitigation.get("execution_result", "planned")).upper()},
                        actions=("inspect",),
                    ),
                ),
                edges=(
                ...
                telemetry=(
                    UiLabTelemetryItem("Packets/sec", _fmt_packets(live.get("packet_rate", 0.0)), "Live packet rate", "good"),
                    UiLabTelemetryItem("Mitigation state", _safe_text(mitigation.get("execution_result", "planned")).upper(), "Latest mitigation state", "warn"),
                ),
                terminal_tabs=tuple(UiLabTerminalTab(host=host, label=host, state="connected" if checks.get("mininet") else "offline", prompt=f"{host}#") for host in LAB_HOSTS),
                action_buttons=(
                ...
                action_status=UiLabActionStatus(
                    ...
                    detail=str(control_manager.latest_status().get("detail", "Awaiting operator command.")),
                    timestamp=str(control_manager.latest_status().get("timestamp", _now_iso())),
                ),
            ),
    ...
    def doctor(self) -> UiPagePayload:
        bundle = self._bundle()
        findings = tuple(item for item in collect_diagnostic_findings() if item.status == "fail")
    ...
    def session(self) -> UiPagePayload:
        ...
        service_rows = tuple(
            (
                ...
                _safe_text(item.get("state", item.get("sync_state", "unknown"))).upper(),
        ...
        return UiPagePayload(
            ...
            tables=(
                ...
                _bundle_table(
                    ...
                    (
                        ("ACTIVE SESSIONS", _fmt_int(len(diagnostics.get("active_sessions", [])))),
                        ("HEARTBEATS", _fmt_int(diagnostics.get("heartbeat_count", 0))),
                        ("REPLAY EVENTS", _fmt_int(diagnostics.get("replay", {}).get("event_count", 0))),
                    ),
                ),
            ),
            updated_at=_now_iso(),
    ...
    def explorer(self, path: str, title: str, payload_path: str, description: str, params: dict[str, Any] | None = None) -> UiPagePayload:
        bundle = self._bundle()
        response = self._api(payload_path, params or {"limit": 25, "offset": 0})["payload"]
        rows = tuple(dict(item) for item in response.get("items", []))
        columns = tuple(sorted({key for row in rows for key in row.keys()}))
        table_rows = tuple(tuple(_safe_text(row.get(column, ""), "") for column in columns) for row in rows)
        return UiPagePayload(
            ...
            status_bar=_status_bar(bundle),
            tables=(
                _table(title.upper(), tuple(column.replace("_", " ").upper() for column in columns), table_rows),
                _bundle_table(
                    "SUMMARY",
                    (
                        ("TOTAL", _fmt_int(response.get("total", len(rows)))),
                        ("REPLAY SAFE", "YES" if bool(response.get("replay_safe", True)) else "NO"),
                        ("TIMESTAMP", _safe_text(response.get("timestamp", _now_iso()))),
                    ),
                ),
            ),
