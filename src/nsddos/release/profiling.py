...
def build_profiling_result(
    ...
    memory_usage_score = round(
        max(0.0, 1.0 - stress_test.memory_pressure_score / 1.5), 4
    )
    cpu_usage_score = round(max(0.0, 1.0 - stress_test.cpu_pressure_score / 1.5), 4)
    slow_function_score = round(
        min((benchmark.benchmark_score + load_test.load_test_score) / 2.0, 1.0), 4
    )
    io_bottleneck_score = round(
        min(
            max(benchmark.streaming_throughput, 0.1)
            / max(load_test.stream_burst_count / 10.0, 1.0),
            1.0,
        ),
        4,
    )
