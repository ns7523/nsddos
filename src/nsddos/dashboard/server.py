...
from nsddos.runtime.policy import (
    latest_history_payload as latest_policy_history_payload,
)
...
def _dashboard_id(environment: str, timestamp: datetime) -> str:
    digest = hashlib.sha256(
        f"{environment}:{timestamp.isoformat()}".encode("utf-8")
    ).hexdigest()[:16]
...
def _load_sources() -> DashboardSourceBundle:
    ...
    return DashboardSourceBundle(
        ...
        policy_history=tuple(
            item
            for item in latest_policy_history_payload().get("entries", [])
        ...
        verification=tuple(
            item
            for item in (latest_run[-1].get("results", []) if latest_run else [])
            if isinstance(item, dict)
        ),
    )
...
def generate_dashboard_state(
    config: dict[str, Any], environment: str = "ops"
) -> DashboardEvaluation:
    ...
    evaluation = DashboardEvaluation(
        ...
        dashboard_health=_dashboard_health(
            diagnostics.missing_data_warnings, len(alerts)
        ),
        timestamp=timestamp,
...
def dashboard_alerts(
    ...
    return tuple(
        item.to_dict()
        for item in generate_dashboard_state(config, environment=environment).alerts
    )
...
def dashboard_report(
    ...
    return tuple(
        item.to_dict()
        for item in generate_dashboard_state(config, environment=environment).reports
    )
...
def dashboard_diagnostics(
    ...
    return generate_dashboard_state(
