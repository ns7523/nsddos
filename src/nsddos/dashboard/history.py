...
def persist_dashboard_history(evaluation: DashboardEvaluation) -> None:
    ...
    with locked_persistence_scope(DASHBOARD_DIR) as lock_scope:
        atomic_write_json(
            DASHBOARD_DIR / f"dashboard-{stamp}.json", payload, lock_scope=lock_scope
        )
        ...
        atomic_write_json(
            DASHBOARD_DIR / "history.json",
            {
                "attack_history": [
                    item.to_dict()
                    for item in evaluation.timeline
                    if item.event_type == "attack_detection"
                ],
                "mitigation_history": [
                    item.to_dict()
                    for item in evaluation.timeline
                    if item.event_type == "mitigation"
                ],
                "cluster_history": [
                    item.to_dict()
                    for item in evaluation.timeline
                    if item.event_type
                    in {"policy_change", "ml_retraining", "ml_inference"}
                ],
            },
            lock_scope=lock_scope,
        )
        atomic_write_json(
            DASHBOARD_DIR / "alerts.json",
            {"alerts": [item.to_dict() for item in evaluation.alerts]},
            lock_scope=lock_scope,
        )
        atomic_write_json(
            DASHBOARD_DIR / "reports.json",
            {"reports": [item.to_dict() for item in evaluation.reports]},
            lock_scope=lock_scope,
        )
        atomic_write_json(
            DASHBOARD_DIR / "diagnostics.json",
