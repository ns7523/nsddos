...
from nsddos.dashboard.contracts import AlertRecord, AttackState, DashboardDiagnostics, DashboardReport, MLMetricsState, PolicyAnalytics
...
def build_reports(
    ...
    return (
        DashboardReport(
            ...
            (("attack_summary", str(attacks.attack_types)), ("alert_summary", str(len(alerts)))),
            timestamp,
        ),
        DashboardReport(
            "report-daily",
            ...
            (("policy_events", str(policy_analytics.policy_events)), ("cluster_nodes", str(cluster_nodes))),
            timestamp,
        ),
        DashboardReport(
            ...
            (("attack_types", str(attacks.attack_types)), ("severity", str(attacks.attack_severities))),
            timestamp,
        ),
        DashboardReport(
            ...
            (("ml_confidence", f"{ml_metrics.ml_confidence:.4f}"), ("warnings", ",".join(diagnostics.missing_data_warnings) or "none")),
            timestamp,
        ),
    )
